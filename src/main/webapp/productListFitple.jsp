	...
	<div id="wrapper">
		...
		<c:if test="${overall.hasOveralls() }">
					...
					<c:if test="${overall.startPage > 5 }">
						<a class="paging"
							href="fitple.do?p_type=${p_type}&pNo=${overall.startPage -5 }&s1=${s1}&s2=${s2}&s3=${s3}&s4=${s4}&s5=${s5}">[이전]</a>
					</c:if>
					<c:forEach var="pNo" begin="${overall.startPage }"
						end="${overall.endPage }">
						<a class="paging" href="fitple.do?p_type=${p_type}&pNo=${pNo }&s1=${s1}&s2=${s2}&s3=${s3}&s4=${s4}&s5=${s5}">[${pNo }]</a>
					</c:forEach>
					<c:if test="${overall.endPage < overall.totalPages }">
						<a class="paging"
							href="fitple.do?p_type=${p_type}&pNo=${overall.startPage+5 }&s1=${s1}&s2=${s2}&s3=${s3}&s4=${s4}&s5=${s5}">[다음]</a>
					</c:if>
					</center>
				</c:if>
